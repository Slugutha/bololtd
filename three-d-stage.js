...
(() => {
  ...
  class ThreeDStage extends HTMLElement {
    constructor() {
      ...
      if (!this.hasAttribute('no-toolbar')) root.appendChild(this._toolbar);
    ...
    async _boot() {
      ...
      renderer.domElement.addEventListener('wheel', (e) => {
        ...
        const factor = e.deltaY < 0 ? dollyScale : 1 / dollyScale;
     ...
     *  from the framed object's center) over `duration` ms, keeping the
     *  same framing distance. Stops autorotate. */
    setView(dirX, dirY, dirZ, zoom = 1, duration = 700) {
      this._flyToView(dirX, dirY, dirZ, duration, zoom);
    }

    /** Fly back to the initial auto-framed view (also triggered by a
     *  double-click/tap on the canvas). */
    resetView(duration = 600) {
      if (!this._homeDir) return;
      this._flyToView(this._homeDir.x, this._homeDir.y, this._homeDir.z, duration, this._homeZoom || 1);
      if (this._controls && this._frameCenter) {
        this._controls.target.copy(this._frameCenter);
      }
    }

    _flyToView(dirX, dirY, dirZ, duration, zoom = 1) {
      const THREE = this._THREE;
      if (!THREE || !this._camera || !this._frameCenter) return;
      this._controls.autoRotate = false;
      const dir = new THREE.Vector3(dirX, dirY, dirZ).normalize();
      const target = this._frameCenter.clone().add(dir.multiplyScalar(this._frameDist * (zoom || 1)));
      if (!duration) {
      ...
      const step = (now) => {
        ...
        this._camera.position.lerpVectors(start, target, e);
        this._controls.update();
    ...
    setObject(object) {
      ...
      if (!box.isEmpty()) {
        ...
        const homeZoom = 0.81;
        const dir = new THREE.Vector3(-0.29, -0.43, 0.85).normalize();
        this._camera.position
          .copy(sphere.center)
          .add(dir.multiplyScalar(dist * homeZoom));
        this._camera.near = Math.max(dist / 100, 0.01);
        this._camera.far = dist * 100;
        this._camera.updateProjectionMatrix();
        this._controls.target.copy(sphere.center);
        this._controls.update();
        this._frameCenter = sphere.center.clone();
        this._frameDist = dist;
        this._homeDir = { x: -0.29, y: -0.43, z: 0.85 };
        this._homeZoom = 0.81;
      ...
      this._flyToView(-0.39, -0.42, 1.49, 0);
    }
